...
Use DGEList to read in counts from the PrepDE.py script output:
```{r}
countData <- as.matrix(read.csv("~/hisatAnalysis/stringtie_prepDE_output/stringtiePrepDE_geneCountMatrix.csv", row.names="gene_id"))
# Load sample information:
sampleInformation <- read.csv("~/hisatAnalysis/ballgownPhenodata.csv")

dgeData <- DGEList(counts = countData, samples = sampleInformation, group = c(rep("1",3),rep("2",3),rep("4",3),rep("5",3),rep("6",3),rep("7",3)))
dgeData
```
...
unlist(lapply(1:3,function(x) dim(qlfSigPTags[[x]]))) # 732     5  6518     5 16820     5
...
write.csv(EdgeR_upRegGenes[[2]],"upregulated in Tbud compared to PNT.csv")
write.csv(EdgeR_upRegGenes[[1]],"upregulated in ConCap compared to InjCap.csv")
